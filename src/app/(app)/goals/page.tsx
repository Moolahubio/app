...
import { goals, wallet } from "@/lib/data";
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          ...
          <Link key={g.id} href={`/goals/${g.id}`} className="group">
            <Card className="h-full p-6 transition-all group-hover:-translate-y-1 group-hover:shadow-card-hover">
              ...
              <div className="mt-1 flex items-center gap-1.5 text-xs text-ink-500">
                ...
                by {new Date(g.deadline).toLocaleDateString("en-US", { month: "long", day: "numeric" })}
              </div>
        ...
        <button className="flex min-h-[280px] flex-col items-center justify-center gap-3 rounded-3xl border-2 border-dashed border-ink-900/15 p-6 text-ink-400 transition-colors hover:border-jade-500/40 hover:text-jade-600 focus-ring">
      ...
      <Card className="flex items-start gap-3 border-sky-500/15 bg-sky-50/50 p-5">
        ...
        <p className="text-sm text-ink-600">
          ...
          <span className="font-semibold text-ink-900">
            {formatMoney(wallet.balanceCents - wallet.allocatedCents)}
          </span>{" "}
