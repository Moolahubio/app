 ...
 * client opens pay.coinbase.com with ONLY that token (the address/assets are
 * encoded in the token, so URL params can't be tampered).
 *
 * Onramp delivers real funds, so it targets Base mainnet — purchased USDC lands
 * in the user's Base wallet and is credited by our on-chain deposit sync.
 */
...
/** Build the hosted Onramp URL from a session token (address/assets are in the token). */
