...
export function streakVisual(status: StreakStatus, count: number): Visual {
  if (status === "frozen") {
    return { Icon: Snowflake, color: "text-sky-500 dark:text-sky-400", glow: "bg-sky-500/15", label: "Frozen" };
  }
  if (status === "paused") {
    return { Icon: Plane, color: "text-muted-foreground", glow: "bg-muted", label: "On vacation" };
  }
  if (status === "broken" || count === 0) {
    return { Icon: Flame, color: "text-muted-foreground", glow: "bg-muted", label: "No streak yet" };
  }
  if (count >= 12) {
    return { Icon: Flame, color: "text-amber-500", glow: "bg-amber-500/15", label: "On fire" };
  }
  return { Icon: Flame, color: "text-jade-500", glow: "bg-jade-500/15", label: "Active" };
}
...
export function StreakIndicator({
  ...
  return (
    <Link
      href="/streaks"
      aria-label={`Savings streak: ${count} ${count === 1 ? "period" : "periods"}`}
      className={cn(
