...
import { formatMoney, avatarSrc, formatDate } from "@/lib/utils";
...
export function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const nav: NavItem[] = [
    ...
    { label: t("nav.personalSavings"), shortLabel: t("nav.personalSavingsShort"), href: "/goals", icon: PiggyBank },
    { label: t("nav.groupSavings"), shortLabel: t("nav.groupSavingsShort"), href: "/circles", icon: UsersRound },
  ...
  return (
    <div className="min-h-[100dvh] bg-background">
      <aside className="glass fixed inset-y-0 start-0 z-30 hidden w-64 flex-col rounded-none border-y-0 border-s-0 px-4 py-6 lg:flex">
        ...
        {reminder && (
          <div className="hover-lift mt-6 rounded-xl border border-white/10 bg-ink-950 p-4 text-white">
        ...
        >
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-xs text-muted-foreground">{t("nav.viewProfile")}</p>
          </div>
        </Link>
      </aside>
          ...
          <div className="flex h-14 items-center justify-between gap-4 px-5 lg:h-16 lg:px-8">
            ...
            <div className="flex items-center gap-2">
              ...
              <Link
                ...
                className="ms-0.5 rounded-full ring-offset-2 ring-offset-background transition-shadow hover:ring-2 hover:ring-jade-500/40 focus-ring lg:hidden"
              >
                {user ? (
